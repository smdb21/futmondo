...
server <- function(input, output) {
  ...
  output$tablaComprasMercado <- DT::renderDataTable(tabla_compras(),
                                                    options = list(
                                                      pageLength = 10
  ...
  output$tablaComprasTopJugadoresFichados <- DT::renderDataTable({
  ...
  output$gastosPlot <- renderPlotly({
    plot <- ggplot(data=t, aes(x=Fecha, y=Precio, group=Comprador, color=Comprador)) +
      geom_line() 
