        ...
        fluidRow(
          ...
          column(
            ...
            box(
              ...
              tags$hr(),
              tags$h4(icon("ranking-star")," Who should I sell?"),
...
players_in_teams_Server <- function(id, is_module_active, login_token, championship_id, user_team_id, user_teams_RV, refresh_trigger = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    # Shared sell-ranking inputs and immutable-ID action routing.
    sell_rank_target_pool_RV <- reactive({
      market<-market_players_RV()
      teams<-user_teams_RV()
      clauses<-data.frame()
      if(is.data.frame(teams)&&nrow(teams)) {
        id_col<-if("teamid"%in%names(teams)) "teamid" else if("id"%in%names(teams)) "id" else NULL
        if(!is.null(id_col)) {
          rivals<-setdiff(as.character(teams[[id_col]]),as.character(user_team_id()))
          rosters<-lapply(rivals,function(team)tryCatch(
            get_players_from_team(login_token(),championship_id(),team),error=function(e)NULL))
          rosters<-Filter(function(x)is.data.frame(x)&&nrow(x),rosters)
          if(length(rosters)) clauses<-today_filter_clause_candidates(
            data.table::rbindlist(rosters,fill=TRUE)%>%as.data.frame(),user_team_id())
        }
      }
      pool<-data.table::rbindlist(list(market,clauses),fill=TRUE)%>%as.data.frame()
    ...
    sell_rankings_RV <- reactive({
      squad<-players_table_RV(); req(is.data.frame(squad),nrow(squad))
      target<-NULL;positions<-NULL
      if(identical(input$sell_rank_mode,"target")) {
        pool<-sell_rank_target_pool_RV(); id<-as.character(input$sell_rank_target)
        target<-pool[as.character(pool$id)==id,,drop=FALSE]
        if(nrow(target)!=1L) return(data.frame())
      } else positions<-input$sell_rank_position
      financial<-financial_snapshot_RV()
      pending<-if(is.list(financial$roster_capacity)) financial$roster_capacity$pending_outbound_ids else character()
      press<-tryCatch(get_championship_pressroom(login_token(),championship_id()),error=function(e)data.frame())
      rank_sale_candidates(squad,positions,target,rules=league_rules_RV(),
        forecast_df=point_forecasts_RV(),pressroom_df=press,pending_outbound_ids=pending,max_results=5L,
        include_all=isTRUE(input$sell_rank_show_all))
    })
    ranking_selected_RV <- reactiveVal(NULL)
    output$sell_ranking_results <- renderUI({
      ...
      cards<-lapply(seq_len(nrow(rows)),function(i) {
        r<-rows[i,,drop=FALSE]
        payload<-jsonlite::toJSON(list(player_id=as.character(r$player_id),
          target_id=if(identical(input$sell_rank_mode,"target")) as.character(input$sell_rank_target) else NULL),
          auto_unbox=TRUE,null="null")
        div(class="sell-ranking-card",
          div(class="sell-ranking-heading",strong(paste0("#",r$rank," ",r$player_name)),
            span(class="sell-ranking-evidence",paste0(r$evidence_pct,"% evidence"))),
          div(class="sell-ranking-meta",span(icon("users")," ",r$remaining_position_counts),
            span(icon("chart-line")," XI ",if(is.finite(r$xi_points_change)) round(r$xi_points_change,1) else "unavailable"),
            span(icon("coins")," ",r$proceeds_type)),
          p(r$rationale),
          div(class="today-recommendation-controls",
            tags$button(type="button",class="btn btn-default",
              onclick=paste0("Shiny.setInputValue(",jsonlite::toJSON(ns("sell_rank_open"),auto_unbox=TRUE),",",payload,",{priority:'event'});"),
              icon("address-card")," Open Player Card"),
            if(identical(input$sell_rank_mode,"target")) tags$button(type="button",class="btn btn-success",
              onclick=paste0("Shiny.setInputValue(",jsonlite::toJSON(ns("sell_rank_apply"),auto_unbox=TRUE),",",payload,",{priority:'event'});"),
              icon("calculator")," Apply to Sandbox")))
      })
    ...
    observeEvent(input$sell_rank_open,{
      event<-input$sell_rank_open; roster<-players_table_RV()
      req(is.list(event),is.data.frame(roster),nrow(roster))
      player<-resolve_sell_ranking_player(event$player_id,roster,sell_rankings_RV())
      if(is.null(player)) {
        showNotification("This player is no longer in your current squad. Refresh and try again.",type="warning")
        return()
      }
      ranking_selected_RV(player)
      showModal(modalDialog(selected_player_UI(ns("sell_rank_player")),
        footer=div(style="text-align:center;width:100%;",modalButton("Close")),easyClose=TRUE,size="l"))
    },ignoreNULL=TRUE)
    selected_player_Server("sell_rank_player",ranking_selected_RV,login_token,
      championship_id,user_team_id,on_bid_updated=function(...) {
        if(!is.null(refresh_trigger)) tryCatch(refresh_trigger(refresh_trigger()+1),error=function(e)NULL)
      })
    observeEvent(input$sell_rank_apply,{
      event<-input$sell_rank_apply; rows<-sell_rankings_RV(); pool<-sell_rank_target_pool_RV()
      req(is.list(event),is.data.frame(rows),is.data.frame(pool))
      selection<-validate_sell_ranking_sandbox_event(event,rows,pool)
      if(is.null(selection)) {
        showNotification("This recommendation is stale. Refresh before applying it.",type="warning")
        return()
      }
      updateSelectizeInput(session,"sandbox_sells",
        choices=stats::setNames(as.character(event$player_id),rows$player_name[match(event$player_id,rows$player_id)]),
        selected=as.character(event$player_id))
      target<-pool[as.character(pool$id)==as.character(event$target_id),,drop=FALSE]
      updateSelectizeInput(session,"sandbox_buys",
        choices=stats::setNames(as.character(event$target_id),as.character(target$name[1])),
        selected=as.character(event$target_id))
    },ignoreNULL=TRUE)
