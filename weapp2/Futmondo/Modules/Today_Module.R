    ...
    fluidRow(
      ...
      column(
        ...
        box(
          ...
          div(class="today-sell-ranking-launcher",
            actionButton(ns("toggle_sell_ranking"),
              label=tagList(icon("ranking-star")," Who should I sell?"),
...
today_Server <- function(id, is_module_active, login_token, championship_id,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      sell_ranking_visible_RV <- reactiveVal(FALSE)
      observeEvent(input$toggle_sell_ranking,{
        visible<-!isTRUE(isolate(sell_ranking_visible_RV()))
        sell_ranking_visible_RV(visible)
        updateActionButton(session,"toggle_sell_ranking",
          label=tagList(icon(if(visible)"xmark" else "ranking-star"),
            if(visible)" Close sell ranking" else " Who should I sell?"))
      },ignoreInit=TRUE)
      ...
      output$sell_ranking_controls <- renderUI({
        req(is_module_active()==TRUE)
        if(!isTRUE(sell_ranking_visible_RV())) return(NULL)
        tagList(
      ...
      observe({
        req(is_module_active()==TRUE,isTRUE(sell_ranking_visible_RV()))
