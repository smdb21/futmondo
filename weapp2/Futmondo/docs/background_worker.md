...
Collection jobs retain accumulated `failure_count` and structured `issues` across cooperative ticks. Each issue contains safe table, collection stage, category, HTTP status and sanitized database error code; raw response bodies and credentials are excluded. `persistence_record_result(job, ok, issues)` evaluates the entire collection and retains the warning until a matching successful retry. Direct HTTP writes require a numeric 2xx result; an unknown result does not prove persistence. This account-wide warning is separate from MVP retrieval and attribution status. Regression coverage: `test/test_mvp_runtime.R` and `test/test_background_sync.R`.
