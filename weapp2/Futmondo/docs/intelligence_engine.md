...
BUY rows additionally expose optional `my_bid_id`, `my_bid_price`, `market_bid_count`, and `market_expires_at`. A nonempty verified own-bid ID plus a finite positive price changes the action to `modify_bid` / **Update Bid**; a visible price without that ID remains `market_bid` / **Place Bid**.
