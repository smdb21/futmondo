...
### Ownership, market status, and own-player clauses

Owned, unlisted players expose **Sell directly to Futmondo**, without requiring an incoming offer. `btn_direct_sell` checks current API ownership and opens a confirmation explaining that sale proceeds are determined by Futmondo. `submit_direct_sell` consumes the single-use `direct_sell` account/league/team/player context and rechecks ownership before calling `direct_sell_player()`. Listed players must first use Remove from Market. Success emits `player_sold_direct`; unknown transport outcomes refresh without announcing a confirmed sale. See [market selling](market_selling.md) for the captured payload and tests.

`player_card_owner_id(player)` accepts a player data frame and returns the first nonblank immutable owner ID from `owner_team_id`, `user_team_id`, `userteamId`, `userTeamId`, or flattened `userTeam`/`userteam` ID aliases; missing IDs return `""`. The card uses it for display and own-player protection, and normalizes its selected row's `user_team_id` before offer/action handlers consume it. Names never determine ownership. Example: `player_card_owner_id(data.frame(user_team_id="", userteamId="rival-7"))` returns `"rival-7"`. Regression coverage is in `test/test_ui_reliability.R`.

`player_card_is_own_player(player, current_team_id)` compares trimmed immutable `user_team_id` values and fails closed for missing IDs. `player_card_can_buy_clause(player, current_team_id, clause_open)` returns `TRUE` only for an open clause on a player outside the current squad. The action renderer therefore never displays **Buy Clause** or a locked-clause purchase badge for an own player. The shared clause-modal route repeats the own-team guard before preflight, preventing stale or external `clause_buyout` events from opening a purchase confirmation for an own player.

`player_card_location_status(player, current_team_id)` returns independent `ownership`, `on_market`, and `market` values. Ownership is `Free Agent`, `Your Squad`, or `Rival Owned[: name]`; market state is separately `Listed on Market` or `Not listed on Market`. An explicit `market_inMarket` value is authoritative. Asking-price fields are fallback evidence only when that flag is absent, and the mere presence of a `computer` column never implies a listing. `player_card_location_label()` formats both dimensions as `Ownership: … · Market: …`.
  ...
  * `open_action` (optional) -- reactive returning a stable action code. When a selected player is currently valid, the module routes the three acquisition codes below (guarded against stale startup reactive values; all other codes, e.g. `"view"`, are ignored):
    ...
    * **`"modify_bid"`** (Today's **Update Bid**) -> re-fetches the player summary, requires an immutable own-team bid ID and finite positive bid price, then opens the same update-bid modal as the player card. Missing/stale evidence fails closed and never falls back to a new bid.
...
Player cards render Place Bid, owner-offer, and Buy Clause controls disabled when effective capacity is exhausted or unavailable, with the same current/outbound/inbound/effective/cap explanation used by server preflight. Verified Update Bid and Cancel Bid controls remain enabled. This is not CSS-only: modal-open and submit handlers recheck capacity to reject forged or stale client events.
