...
* `today_UI(id)` -- Renders the Manager Command Center layout with a hero banner, 4 KPI value boxes, a 2-column main area (recommendations feed + market radar / recent deals). The recommendations box carries a rival-listing control for acquisition views: it is available for **Buy players**, turns on automatically for a **Clause-only** view, and is inactive for Sell/Hold-only views.
...
* For **Buy players**, the unchecked **"Include rival-owned market listings"** toggle (`include_rival_listings`) opts in to also showing **rival-owned** market listings. A **Clause-only** filter enables these listings automatically; Sell/Hold-only filters exclude them. (a resolvable immutable owner ID that is not the current team). **Unknown-owner listings are always hidden**, in both modes.
...
* BUY cards show a verified current outbound bid and total bid count when supplied by the normalized market row. Their action becomes **Update Bid** (`modify_bid`); a market price without an own `bid_id` is never treated as the user's bid.
* BUY cards render `expirationDate` as a live one-second countdown (`HH:MM:SS`, prefixed with days when needed). Missing dates show **Expiry unavailable**. At zero they show **Market closed** and disable the mutation button until normal data refresh removes the stale card.
...
1. **Resolution (action-aware, candidate-based)** -- `selected_today_player_RV` resolves `event$player_id` via the pure helper `today_resolve_player_for_action(player_id, action, market_df, all_df, clause_df = ...)`:
   ...
   * **`"modify_bid"` (Update Bid)** resolves from the same current filtered market candidates, then the selected-player module re-fetches the player summary and opens the existing update modal only when an immutable own-team bid ID and positive price are still verified.
...
2. **Modal** -- an observer on `selected_from_today_RV` consumes the SAME action-aware resolved reactive (`selected_today_player_RV`; it does **not** re-resolve against `all_players_RV()`) and shows `selected_player_UI(id = ns("selected_player"))` in a Today-local `modalDialog` when the player resolves. When the id is absent from the appropriate candidate set for the action, a clear warning is shown and **no modal** (player, market-offer, or clause-buyout) is opened:
   ...
   * `"view"` actions first use the general player data, then fall back to the current clause and filtered-market candidates. This lets a read-only player card open for a clause candidate fetched from a rival roster.
...
3. **Nested module** -- `today_Server` instantiates a nested `selected_player_Server(id = "selected_player", selected_player = selected_today_player_RV, login_token = ..., championship_id = ..., user_team_id = ..., on_bid_updated = ..., open_action = ...)` under the Today namespace (inputs/UI live under `today-selected_player-...`).
   * `open_action` is the optional reactive carrying the stable action code (`today_open_action_RV`). Inside `selected_player_Server`, an observer routes the three acquisition codes when a selected player is currently valid (guarded against stale startup reactive values):
     * **`"modify_bid"`** -> re-verifies the own bid from the player summary and opens the existing update-bid modal; stale evidence fails closed.
...
BUY and Clause rows include stable `action_disabled` and `action_disabled_reason` fields. Capacity-blocked recommendations stay visible with the effective-roster explanation, but their buttons have a real disabled attribute and emit no client action. `modify_bid`, Sell, and Hold actions remain available.
...
The actionable feed places compact icon checkboxes above the cards for **Buy players**, `Sell`, **Offers received**, `Clause`, and `Hold`. **Buy players** means a suggested new market acquisition; **Offers received** means another manager has bid for a player already in the user's squad, so it is a sell/accept decision. All types are selected initially. `today_filter_recommendations(recommendations, selected_types)` returns matching rows in their original priority order; clearing every type displays the existing empty-feed state. Bid and clause amounts use Spanish thousands separators and the euro symbol, for example `1.234.567 €`.
