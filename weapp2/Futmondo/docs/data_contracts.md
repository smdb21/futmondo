...
Temporary acquisition credit is verified from the current team value. `acquisition_headroom(cash,team_value,withheld=0,commitments=0,debt_fraction=0.5)` permits spending down to `minimum_balance = -0.5 * team_value`, while subtracting the larger of held funds and outgoing bids, because matching values describe one reservation. It returns `spendable_budget`, `debt_limit`, `minimum_balance`, `projected_committed_balance`, and `reserved_amount`; unknown inputs remain unavailable. `get_acquisition_capacity()` and `get_financial_snapshot()` expose these fields. Positive cash is still required at the next round start to score points.
